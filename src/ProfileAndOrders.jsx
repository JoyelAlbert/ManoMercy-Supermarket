...
const ProfileAndOrders = () => {
  ...
  useEffect(() => {
    const fetchProfileAndOrders = async () => {
      try {
        ...
        const profileRes = await axios.get("https://manomercy-supermarket.onrender.com/api/v1/users", {
          headers: { Authorization: `Bearer ${token}` },
        });

        const ordersRes = await axios.get("https://manomercy-supermarket.onrender.com/api/v1/orders", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const res = await axios.put(
        `https://manomercy-supermarket.onrender.com/api/v1/users/${profile._id}`,
        editData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(
        `https://manomercy-supermarket.onrender.com/api/v1/users/${profile._id}/password`,
        {
