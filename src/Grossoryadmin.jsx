...
function Grossoryadmin() {
  const [showModal, setShowModal] = useState(false);
  const [editId, setEditId] = useState(null);
  const [search, setSearch] = useState("");
  const [activeTab, setActiveTab] = useState("grossory_admin");
  ...
  const {
    ...
    toggleHomeProduct,
    homeProducts
  } = useProducts("http://localhost:5001/api/v1/grossory_products");
  ...
  // Subcategories for Grossory
  const grossorySubcategories = ["Rice", "Oil", "Spices", "Flour", "Dhal", "Other"];

  return (
    <div className="grossory-home-container">
      {/* Loading State */}
      {isLoading && (
        <div className="page-loading admin-loading">
          <i className="fas fa-boxes fa-spin"></i>
          <h3>Loading Grossory Products</h3>
          <p>Managing your grossory products...</p>
        </div>
      )}

      {/* Title Bar */}
      <div className="grossory-home-title-bar">
        <div className="title-left">
          <h1>Grossory Products Admin</h1>
          <p>Total Products: {allProducts.length} | Home Products: {homeProducts.length}/8</p>
        </div>
        <div className="title-center">
          <div className="grossory-home-title-bar-search-bar">
            <i className="fas fa-search"></i>
            <input
              type="search"
              placeholder="Search grossory products..."
              value={search}
      ...
      {/* Category Tabs */}
      <div className="grossory-home-tabs">
        {categories.map((cat) => (
          <button
            key={cat.key}
            onClick={() => {
      ...
      {/* Product List */}
      <div className="grossory-home-list">
        {filteredProducts.length === 0 ? (
          <div className="no-products">
            <i className="fas fa-boxes"></i>
            <h3>No Grossory Products Found</h3>
            <p>Try adding some products or adjusting your search</p>
          </div>
        ) : (
          filteredProducts.map((p) => (
            <div className={`grossory-home-card ${!p.inStock ? "out-of-stock" : ""}`} key={p._id}>
              ...
              <div className="grossory-home-card-content">
                <h2>{p.name}</h2>
              ...
              <div className="grossory-home-actions">
                <button 
                  className="edit" 
      ...
      {/* Modal */}
      {showModal && (
        <div className="grossory-home-modal" onClick={() => setShowModal(false)}>
          <div className="grossory-home-modal-content" onClick={(e) => e.stopPropagation()}>
            <span 
              className="grossory-home-close" 
              onClick={() => setShowModal(false)}
            >
              &times;
            </span>
            
            <h2>{editId ? "Edit Grossory Product" : "Add New Grossory Product"}</h2>
