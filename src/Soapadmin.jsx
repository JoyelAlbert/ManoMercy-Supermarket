...
function Soapadmin() {
  ...
  const [activeTab, setActiveTab] = useState("soap_admin");
  ...
  // Subcategories for Soap
  const soapSubcategories = ["Soap", "Shampoo", "Lotion", "Other"];

  return (
    <div className="soap-home-container">
      {/* Loading State */}
      {isLoading && (
        <div className="page-loading admin-loading">
          <i className="fas fa-pump-soap fa-spin"></i>
          <h3>Loading Soap Products</h3>
          <p>Managing your soap products...</p>
        </div>
      )}

      {/* Title Bar */}
      <div className="soap-home-title-bar">
        <div className="title-left">
          <h1>Soap Products Admin</h1>
          <p>Total Products: {allProducts.length} | Home Products: {homeProducts.length}/8</p>
        </div>
        <div className="title-center">
          <div className="soap-home-title-bar-search-bar">
            <i className="fas fa-search"></i>
            <input
              type="search"
              placeholder="Search soap products..."
              value={search}
      ...
      <div className="soap-home-list">
        {filteredProducts.length === 0 ? (
          ...
          filteredProducts.map((p) => (
            <div className={`soap-home-card ${!p.inStock ? "out-of-stock" : ""}`} key={p._id}>
              ...
              <div className="soap-home-card-content">
                <h2>{p.name}</h2>
              ...
              <div className="soap-home-actions">
                <button 
                  className="edit" 
      ...
      {/* Modal */}
      {showModal && (
        <div className="soap-home-modal" onClick={() => setShowModal(false)}>
          <div className="soap-home-modal-content" onClick={(e) => e.stopPropagation()}>
            <span 
              className="soap-home-close" 
              onClick={() => setShowModal(false)}
            >
              &times;
            </span>
            
            <h2>{editId ? "Edit Soap Product" : "Add New Soap Product"}</h2>
