...
function Soap() {
  ...
  const {
    ...
    error,
    fetchProducts
  } = useProducts("https://manomercy-supermarket.onrender.com/api/v1/soap_products");
  ...
  const mainCategory = "Soap";
  const categoriesMap = {
    Soap: ["All", "Soap", "Shampoo", "Lotion", "Other"],
  };
  ...
  const getOrCreateDraftOrder = async (token) => {
    try {
      ...
      const ordersRes = await fetch("https://manomercy-supermarket.onrender.com/api/v1/orders", {
        headers: { 
      ...
      const createRes = await fetch("https://manomercy-supermarket.onrender.com/api/v1/orders/draft", {
        method: "POST",
        headers: { 
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const handleConfirmOrder = async (product, qty) => {
    ...
    try {
      ...
      const addItemRes = await fetch(`https://manomercy-supermarket.onrender.com/api/v1/orders/${draftOrder._id}/add-item`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="soap-user-container">
      {/* Loading State */}
      {isLoading && (
        <div className="page-loading products-loading">
          <i className="fas fa-pump-soap fa-spin"></i>
          <h3>Loading Soap Products</h3>
          <p>Getting hygiene products for you...</p>
        </div>
      )}
      ...
      {!isLoading && !error && (
        <>
          {/* ================= Title Bar ================= */}
          <div className="soap-user-title-bar">
            <div className="title-left">
              <h1>ManoMercy Supermarket</h1>
            </div>
            <div className="title-center">
              <div className="soap-user-title-bar-search-bar">
                <i className="fas fa-search"></i>
                <input
                  type="search"
                  placeholder="Search for soap products..."
                  value={search}
          ...
          {/* ================= Product List ================= */}
          <div className={`soap-user-list ${filteredProducts.length <= 3 ? "center-cards" : ""}`}>
            {filteredProducts.length === 0 ? (
              <div className="no-products">
                <i className="fas fa-search"></i>
                <h3>No Soap Products Found</h3>
              ...
              filteredProducts.map((p) => {
                ...
                return (
                  <div
                    className={`soap-user-card ${outOfStock ? 'out-of-stock' : ''}`}
                    key={p._id}
                  >
                    ...
                    <div className="soap-user-actions">
                      <button 
          ...
          {showPopup && selectedProduct && (
            <div className="soap-user-modal">
              <div className="soap-user-modal-content">
                <span 
                  className="soap-user-close" 
